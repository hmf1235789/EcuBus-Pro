                    ...
                    <VxeGrid v-bind="childGridOptions" :data="parent.row.childList">
                        <template #physical="{ row }">
                            <el-input v-if="hasPhysicalEncoding(row.encodingType)"
                                v-model="physicalValues[`${parent.row.name}-${row.name}`]" size="small"
                                @change="handlePhysicalValueChange($event, parent.row.name, row)"
                                style="width: 100px;" />
                            <span v-else>-</span>
                        </template>
                        <template #raw="{ row }">
                            <el-input v-model="rawValues[`${parent.row.name}-${row.name}`]" size="small"
                                @change="handleRawValueChange($event, parent.row.name, row)" style="width: 100px;" />
                        </template>
