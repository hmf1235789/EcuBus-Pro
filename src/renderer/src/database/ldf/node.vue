<template>
    <div>
        <VxeGrid ref="xGrid" v-bind="gridOptions" @cell-click="cellClick">
            ...
            <template #default_supplierId="{ row }">
                0x{{ getSupplierId(row.name) }}
            </template>

            <template #default_functionId="{ row }">
                0x{{ getFunctionId(row.name) }}
            </template>

            <template #default_variant="{ row }">
                0x{{ getVariant(row.name) }}
            </template>
...
function addNewSlaveNode() {
    ElMessageBox.prompt('Please enter the slave node name', 'Add Slave Node', {
        ...
        inputValidator: (value) => {
            ...
            if (value === ldfObj.value.node.master.nodeName) return 'Slave node name can\'t equal master node name'
            return true
        }
    }).then(({ value }) => {
